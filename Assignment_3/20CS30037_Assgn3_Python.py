#  DBMS Lab Assignment 3 - python code for queries
#  Name: Pranav Nyati
#  Roll No: 20CS30037
...
# Function to print the query results in a tabular format
def print_result(result):
    data_table = PrettyTable(result[0])
    for row in result[1:]:
        data_table.add_row(row)
    print(data_table)

def choose_query(input_query):
    ...
    result_dict[4] = ["Patient Name", "Patient Address"]
    ...
    result_dict[5] = ["Patient Name", "Patient Insurance ID"]
    ...
    result_dict[7] = ["Nurse Name", "Nurse Position", "Physician Name"]
    ...
    query_dict[10] = """
                     ...
                     INNER JOIN Trained_In ON Trained_In.PhysicianID = Undergoes.PhysicianID and Trained_In.ProcedureCode = Undergoes.ProcedureCode and Undergoes.ProcedureDate > Trained_In.CertificationExpires;                
                     """
    result_dict[10] = ["Physician Name", "Procedure Name", "Procedure Date", "Patient Name"]
    ...
    query_dict[11] = """
                    ...
                    WHERE mp1.cost > 5000;
                    """
    result_dict[11] = ["Patient Name", "Physician Name"]
    ...
    result_dict[12] = ["Medicine Name", "Medicine Brand"]
    ...
    result_dict[13] = ["Physician Name"]
    
    return query_dict[input_query], result_dict[input_query]
...
try:
    ...
    if connection.is_connected():
        ...
        while (True):
            print("Enter a query number from 1 to 13 OR Enter -1 to exit:")
            input_query = int(input())

            if input_query == -1:
                break

            query, query_attr = choose_query(input_query)
            cursor.execute(query)
            result = list(cursor.fetchall())

            if (len(result) == 0):
                print("The given database has no match corresponding to this query!")
                print("\n")
            elif (len(result) > 0):
                print("The result of the query is: \n")
                temp = []
                temp.append(query_attr)
                temp.extend(result)
                print_result(temp)
                print("\n")
